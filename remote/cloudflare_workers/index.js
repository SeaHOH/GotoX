...
const about = `
/******************************************************************************
 *  GotoX remote server 0.7 in CloudFlare Workers
 ...
 *             Body
 *        +------------+
 *        |            |           a number in uint16, the 1st bit marked
 *        | 2 bytes    |        <- whether the request metadata is deflated,
 *        |            |           and rest marked its length
 *        +------------+
 *        | some bytes |        <- request metadata (raw deflated or not)
 *        +------------+
        ...
        case '/gh/':
            ...
            try {
                ...
                const response = await fetch(await readRequest(request, fetchOptions), {
                    ...
                    cf: {
                        apps: false,
                        scrapeShield: false,  // 设置无效果，电邮地址仍然会混淆
                        polish: 'off',
...
async function readRequest(request, fetchOptions) {
    ...
    let requestMetedataLength = new DataView(chunk.buffer, 0, 2).getUint16()
    const deflated = requestMetedataLength > 0x8000
    requestMetedataLength &= 0x7FFF
    let requestMetedataBytes = new Uint8Array(requestMetedataLength)
    ...
    do {
        if (chunk.length <= left) {
            requestMetedataBytes.set(chunk, offset)
            offset += chunk.length
            left -= chunk.length
            chunk = nullChunk
        }
        else {
            requestMetedataBytes.set(chunk.subarray(0, left), offset)
    ...
    // 转换
    if (deflated)
        requestMetedataBytes = inflateRaw(requestMetedataBytes)
    const requestMetadata = new TextDecoder().decode(requestMetedataBytes)
...
/*
 * Alternate for pako.inflateRaw without exceptions
 * Inflate input at once
 * @param {Uint8Array} bytes
 * @return {Uint8Array}
 */
