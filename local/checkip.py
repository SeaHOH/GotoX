...
主要是检查这个ip是否可以连通，并检查服务端是否为 gws 或 gvs 1.0
"""
...
#最大IP延时，单位毫秒
g_maxhandletimeout = 40000
#需要得到的可用IP数量
g_maxhandleipcnt = 2000
#每轮扫描的IP数量
g_maxthreads = 128

#连接超时设置，单位秒
g_conntimeout = 2
g_handshaketimeout = 4
#SSL 连接：0使用 OpenSSL，1使用 gevent
g_usegevent = 1

#屏蔽列表
#g_blockiplist = ()
g_blockiplist = []#["74.125.","173.194."]#, "64.233."]
#g_blockiplist = ("1.255", "202.248", "222.251", "219.124", "188.43", "173.194" )
...
try:
    ...
    pass
g_ipfile = os.path.join(g_filedir, "ip_new.txt")
...
g_blockiplistfile = os.path.join(g_filedir,"blockip.txt")
...
if os.path.exists(g_blockiplistfile):
    with open(g_blockiplistfile,"r") as fd:
        for line in fd:
            ip = line.strip("\r\n")
            g_blockiplist.append(ip + '.')
...
def isgoogleserver(svrname):
    ...
    if lowerdomain == "gws" or lowerdomain.startswith("gvs"):
        return True
    else:
        return False
...
class my_ssl_wrap(object):
    ...
    def initsslcxt():
        ...
        try:
            ...
            if my_ssl_wrap.ssl_cxt is not None:
                return
            my_ssl_wrap.ssl_cxt = OpenSSL.SSL.Context(OpenSSL.SSL.TLSv1_2_METHOD)
            my_ssl_wrap.ssl_cxt.set_timeout(g_handshaketimeout)
            my_ssl_wrap.ssl_cxt.set_cipher_list(my_ssl_wrap.openssl_ciphers)
            PRINT(u"SSL 环境初始化完毕")
        except Exception:
            raise
        finally:
    ...
    def getssldomain(self, ip):
        ...
        try:
            ...
            if g_useOpenSSL:
                ...
                s.settimeout(g_conntimeout)
                c = ssl.wrap_socket(s, ciphers=self.ssl_ciphers, cert_reqs=ssl.CERT_REQUIRED, ca_certs=g_cacertfile,
    ...
    def runJob(self):
        ...
        while not self.cacheResult.queryfinish():
            try:
                ...
                matchblock = False
                for blockitem in g_blockiplist:
                    if ipaddr.startswith(blockitem):
                        matchblock = True
                        break
                if matchblock:
                    PRINT(u"跳过 ip: %s" % (ipaddr))
                    continue
                (ssldomain,costtime,timeout,gwsname) = ssl_obj.getssldomain(ipaddr)
...
def list_ping():
    ...
    for ip in ip_list:
        matchblock = False
        for blockitem in g_blockiplist:
            if str(ip[1]).startswith(blockitem):
                matchblock = True
                break
        if matchblock:
            continue
